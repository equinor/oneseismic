...
public:
  Server(int maxconn, const std::string &k, const std::string &sa)
      : cm(maxconn), key(k), storage_account(sa) {}
...
std::int64_t indexof(const Geometry &g, std::size_t dim, std::int64_t lineno) {
  ...
  default:
    throw std::invalid_argument("expected dimension [0, 1, 2], was " +
                                std::to_string(dim));
  }
  ...
  if (itr == end) {
    throw std::invalid_argument("No such lineno " + std::to_string(lineno));
  }
...
void assert_dim_positive(int x, int dim) {
  if (x >= 0)
    return;
  throw std::runtime_error("expected fragment size in dim" +
                           std::to_string(dim) + " >= 0, was " +
                           std::to_string(x));
}

std::vector<float> slice_fragment(const std::vector<std::uint8_t> &fragment,
                                  sc::slice_layout lay, std::size_t pin) {
...
grpc::Status Server::Slice(grpc::ServerContext *ctx,
                           const oneseismic::SliceRequest *req,
                           oneseismic::SliceReply *writer) {
  ...
  try {
    ...
    const auto fragment_dimensions = sc::FS<3>{
        std::size_t(req->dim0()), std::size_t(req->dim1()),
        std::size_t(req->dim2()),
    };
    const auto cube_dimensions =
        sc::CS<3>{std::size_t(req->geometry().dim0().size()),
                  std::size_t(req->geometry().dim1().size()),
                  std::size_t(req->geometry().dim2().size())};
    auto gvt = sc::gvt<3>(cube_dimensions, fragment_dimensions);

    auto generator = sc::azure_request_generator(this->storage_account,
                                                 req->geometry().guid(), "src",
...
int main(int argc, char **argv) {
  ...
  std::cout << "Setting up server on:" << address << std::endl;

  // key = env["AZURE_ACCESS_KEY"]

  auto cli =
      clara::Opt(address, "bind")["-b"]["--bind"](
          "Address to bind, default = " + address) |
      clara::Opt(key, "key")["-k"]["--key"]("Access key") |
      clara::Opt(storage_account,
                 "storage account")["-A"]["--account"]["--storage-account"](
  ...
  std::cout << "Connect to account: " << storage_account << std::endl;
