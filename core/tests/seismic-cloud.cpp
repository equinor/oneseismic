...
TEST_CASE("Projecting a dimension-0 slice") {
...
TEST_CASE("Projecting a dimension-1 slice") {
    const auto expected = std::vector< unsigned char > {
        0x0, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0x0,
    ...
    const auto slice_frag_dim = sc::frag_dimension< 3 > { 3, 1,  7 };
    const auto slice_dim      = sc::cube_dimension< 3 > { 3, 1, 14 };
    auto gvt = sc::gvt< 3 >(slice_dim, slice_frag_dim);
    REQUIRE(expected.size() == gvt.global_size() * sizeof(float));

    /* extract a slice from a fragment */
    const auto pin = 1;
    const auto source_stride = exdims.slice_stride(sc::dimension< 3 >(1));
    const auto source = slice(source_stride, pin);
    ...
    /* Put the slice tile at the right place in the output array */
    const auto id = sc::fragment_id< 3 > { 0, 0, 1 };
    auto stride = gvt.slice_stride(sc::dimension< 3 >(1), id);
    auto src = source.begin();
    auto dst = out.begin() + stride.start;
    for (auto i = 0; i < stride.readcount; ++i) {
        std::copy_n(src, stride.readsize, dst);
...
TEST_CASE("Projecting a dimension-1 slice lateral") {
    const auto expected = std::vector< unsigned char > {
        0x0, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0x0,
    ...
    const auto slice_frag_dim = sc::frag_dimension< 3 > { 3, 1, 7 };
    const auto slice_dim      = sc::cube_dimension< 3 > { 6, 1, 7 };
    auto gvt = sc::gvt< 3 >(slice_dim, slice_frag_dim);
    REQUIRE(expected.size() == gvt.global_size() * sizeof(float));
    ...
    /* Put the slice tile at the right place in the output array */
    const auto id = sc::fragment_id< 3 > { 1, 0, 0 };
    auto stride = gvt.slice_stride(sc::dimension< 3 >(1), id);
    auto src = source.begin();
    auto dst = out.begin() + stride.start;
    for (auto i = 0; i < stride.readcount; ++i) {
...
TEST_CASE("Projecting a dimension-2 slice") {
    const auto expected = std::vector< unsigned char > {
        0x0, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0x0,
    ...
    const auto slice_frag_dim = sc::frag_dimension< 3 > { 3, 5, 1 };
    const auto slice_dim      = sc::cube_dimension< 3 > { 6, 5, 1 };
    auto gvt = sc::gvt< 3 >(slice_dim, slice_frag_dim);
    REQUIRE(expected.size() == gvt.global_size() * sizeof(float));

    /* extract a slice from a fragment */
    const auto pin = 1;
    const auto source_stride = exdims.slice_stride(sc::dimension< 3 >(2));
    const auto source = slice(source_stride, pin);
    ...
    /* Put the slice tile at the right place in the output array */
    const auto id = sc::fragment_id< 3 > { 1, 0, 0 };
    auto stride = gvt.slice_stride(sc::dimension< 3 >(2), id);
    auto src = source.begin();
    auto dst = out.begin() + stride.start;
    for (auto i = 0; i < stride.readcount; ++i) {
        std::copy_n(src, stride.readsize, dst);
