...
func TestOnbehalfRetrySuccessfulCalledOnce(t *testing.T) {
	calls := 0
	fn := func(auth string) (interface{}, error) {
		calls += 1
		return auth, nil
	}

	expected := "some-authorization"
	ret, err := WithOnbehalfAndRetry(
		&TokensIdentity{},
		expected,
		fn,
	)
	...
	if ret != expected {
		t.Errorf("Want '%v', got %v", expected, ret)
	}
	if err != nil {
		t.Errorf("Error was %v; want <nil>", err)
	}
}

func TestOnbehalfRetrySuccessfulOnRetry(t *testing.T) {
	calls := 0
	fn := func(auth string) (interface{}, error) {
		calls += 1
		if calls == 2 {
		...
		return nil, fmt.Errorf("Failed attempt")
	}
...
func TestOnbehalfStopsAfterRetries(t *testing.T) {
	calls := 0
	fn := func(auth string) (interface{}, error) {
		calls += 1
		return nil, fmt.Errorf("Failed attempt")
